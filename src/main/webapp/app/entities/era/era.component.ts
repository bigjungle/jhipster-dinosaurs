...
currentAccount: any;
    ...
    loadAll() {
        this.eraService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<Era[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/era'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Era) {
        return item.id;
    }
    registerChangeInEras() {
        this.eventSubscriber = this.eventManager.subscribe('eraListModification', (response) => this.loadAll());
    }
