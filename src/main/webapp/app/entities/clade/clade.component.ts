...
export class CladeComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.cladeService
            ...
            .subscribe(
                (res: HttpResponse<IClade[]>) => this.paginateClades(res.body, res.headers),
    ...
    trackId(index: number, item: IClade) {
        return item.id;
    }

    registerChangeInClades() {
        this.eventSubscriber = this.eventManager.subscribe('cladeListModification', response => this.loadAll());
    }
    ...
    private paginateClades(data: IClade[], headers: HttpHeaders) {
